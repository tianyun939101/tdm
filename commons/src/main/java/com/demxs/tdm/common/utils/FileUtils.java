...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static void zipFiles(String srcDirName, String fileName,
								String descFileName) {
		String aa = System.getProperty("sun.jnu.encoding");
		// 判断目录是否存在
		if (srcDirName == null) {
	...
	public static boolean unZipFiles(String zipFileName, String descFileName) {
        ...
        try {
			// 根据ZIP文件创建ZipFile对象
			ZipFile zipFile = new ZipFile(zipFileName, System.getProperty("sun.zip.encoding"));
			ZipEntry entry = null;
	...
	public static String downFile(File file, HttpServletRequest request, HttpServletResponse response, String fileName){
		...
		try {
			response.addHeader("Content-Disposition", "attachment; filename=\"" + 
					Encodes.urlEncode(StringUtils.isBlank(fileName) ? file.getName() : fileName ).replaceAll("\\+","%20") + "\"");
	 ...
	 */
	public static String getFileContent(String filePath, String encode) throws FileNotFoundException {
		...
		return readFileByLine(bufSize, fcin, rBuffer, wBuffer,encode);
	}

	public static String readFileByLine(int bufSize, FileChannel fcin, ByteBuffer rBuffer, ByteBuffer wBuffer, String encode){
		String enter = "\n";
...
//              System.out.println(line);
	 ...
	 * @return
	 */
	public static String getAttachMentFilePath(String fileType){
		//拼装附件存储路径
		...
		//修正路径
		return FileUtils.path(filePath);
	}
	...
	public static File createTempFile(String fileType){
		try {
			...
			String path = tmpDir.getPath()+File.separator+ UUID.randomUUID()+"."+fileType;
	 ...
	 * @return
	 */
	public static String createTempFile(String fileName,InputStream inputStream){
		try {
			File tmpDir = FileUtils.getTempDirectory();
			String path = tmpDir.getPath()+File.separator+fileName;
	...
	public static String getClassesPath(){
		String path=StringUtils.trimSufffix(SpringContextHolder.getRealPath("/"), File.separator)   +File.separator +"WEB-INF"+File.separator+"classes"+File.separator;
