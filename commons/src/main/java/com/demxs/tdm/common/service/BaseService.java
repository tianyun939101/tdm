...
public abstract class BaseService {
	...
	public static String dataScopeFilter(User user, String officeAlias, String userAlias) {
		...
		List<String> allLabIds = SetUniqueList.decorate(new ArrayList<>());
		// 超级管理员，跳过权限过滤
		if (!user.isAdmin()){
				...
				for (String oa : StringUtils.split(officeAlias, ",")){
					if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)){
						if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())){
							...
							ISystemService systemService = SpringContextHolder.getBean(ISystemService.class);
							List<Map<String,String>> labs = systemService.getLabByUserRole(user.getId(),r.getId());
							if(CollectionUtils.isNotEmpty(labs)){
								for(Map m:labs){
									labIds.add((String)m.get("labId"));
								}
							}
//                            ILabInfoService labInfoService = SpringContextHolder.getBean(ILabInfoService.class);
//                            Map<String, String> labInfo = labInfoService.getLabByUserId(user);
//                            if(labInfo == null || labInfo.isEmpty()){
//
//                            }else{
//                                labIds.add((String)labInfo.get("id"));
//                            }

							allLabIds.addAll(labIds);
...
//							}
			...
			if(allLabIds==null || allLabIds.size()<=0){

				if (sqlString==null|| sqlString.length()<=0) {
					sqlString.append(" AND 1=0");
				}
			}else{
				for(String id :allLabIds){
					sqlString.append(" OR  lab.id = '" + id + "'");
				}
			}
