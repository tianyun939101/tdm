...
public class ZyTestTaskService extends  CrudService<ZyTestTaskDao, ZyTestTask> {

    @Autowired
    ZyTestTaskDao zyTestTaskDao;
    ...
    public Page<ZyTestTask> list(Page<ZyTestTask> page, ZyTestTask entity) {
        Page<ZyTestTask> Page = super.findPage(page, entity);
        return Page;
    }

    public Page<ZyTestTask> findPage(Page<ZyTestTask> page, ZyTestTask entity) {
        entity.setPage(page);
    ...
    public ZyTestTask get(String id) {
        ZyTestTask equipment = super.dao.get(id);
        return equipment;
    }


    public void save(ZyTestTask entity) {
        super.save(entity);
        if("1".equals(entity.getFlag())){
            User auditUser = UserUtils.getUser();
            String title = "请处理'"+UserUtils.getUser().getName()+"'提交的软件构型状态申请！";
            Map<String,Object> vars = new HashMap<>(1);
            vars.put("message", title);
            actTaskService.apiStartProcess("testTask", auditUser.getLoginName(), entity.getId(), title, vars);
    ...
    public void approve(ApproveDTO approveDto) {
        //获取数据
        ZyTestTask zyTestTask = this.get(approveDto.getId());
        ...
        //审批
        String title = "请处理'"+zyTestTask.getCreateBy().getName()+"'提交的软件构型状态检查申请！";
        vars.put("message", title);
    ...
    public void reject(ApproveDTO approveDto) {
        ...
        Map<String,Object> vars = new HashMap<>(1);
        //审批
        String title = "您提交的软件构型状态检查申请，已被'"+UserUtils.getUser().getName()+"'驳回！";
        vars.put("message", title);
        String assignee = auditUser== null ? "" : auditUser.getLoginName();
