...
import com.demxs.tdm.dao.cms.CategoryDao;
import com.demxs.tdm.domain.cms.Banner;
...
import com.demxs.tdm.domain.cms.Category;
...
//		return page;
	...
	public Page<Category> findModule(Page<Category> page, Category category) {
		category.setPage(page);
		page.setList(dao.findModule(category));
		return page;
	}

	public Page<Banner> findInBannerCategory(Page<Banner> page, Banner banner) {
		banner.setPage(page);
		page.setList(dao.findInBannerCategory(banner));
		return page;
	}

	public Banner findInBannerCategoryById(Banner banner) {
		banner = dao.findInBannerCategoryById(banner);
		return banner;
	}

	public Integer removeBanner(String id,String siteId){
		Integer result = 0;
		Banner banner = new Banner();
		banner.setId(id);
		banner.setSiteId(siteId);
		//获取轮播图对象
		banner = this.findInBannerCategoryById(banner);
		//更新对应表单
		if(banner != null)
			result = dao.removeBanner(banner);
		return result;
	}

	public Integer addBanner(String id,String siteId){
		Integer result = 0;
		Banner banner = new Banner();
		banner.setId(id);
		banner.setSiteId(siteId);
