package com.demxs.tdm.service.cms.utils;
...
import com.demxs.tdm.domain.cms.*;
import com.demxs.tdm.service.cms.ArticleService;
import com.demxs.tdm.service.cms.CategoryService;
import com.demxs.tdm.service.cms.LinkService;
import com.demxs.tdm.service.cms.SiteService;
...
public class CmsUtils {
	...
	public static List<Category> getMainNavList(String siteId){
		...
		if (mainNavList == null){
			...
			for(Category  c:list){
				if(c.getName().equals("中心概况")){
					List<Category> ls=categoryService.findByParentId(c.getId(),siteId);
					c.setCategoryList(ls);
	...
	public static List<Category> getCategoryList(String siteId, String parentId, int number, String param){
		...
		Category category = new Category();
		category.setSite(new Site(siteId));
		category.setParent(new Category(StringUtils.isEmpty(parentId)?"1":parentId));
		if (StringUtils.isNotBlank(param)){
			@SuppressWarnings({ "unused", "rawtypes" })
			Map map = JsonMapper.getInstance().fromJson("{"+param+"}", Map.class);
		}
		category.setInList("1");
		page = categoryService.findModule(page, category);
		return page.getList();
	}
	 ...
	 * @return
	 */
    public static List<Category> getImageList(String siteId, String parentId,int number){
        Page<Category> page = new Page<Category>(1, number, -1);
        Category category = new Category();
        category.setSite(new Site(siteId));
		category.setParent(new Category(parentId));
		category.setModule("picture");
        page = categoryService.findModule(page, category);
        return page.getList();
    }
	 ...
	 * @return
	 */
	public static List<Banner> getBackgroundImg(String siteId){
		Page<Banner> page = new Page<Banner>(1, 1000, -1);
		Banner banner = new Banner();
		banner.setSiteId(siteId);
		banner.setInBanner("1");
		page = categoryService.findInBannerCategory(page, banner);
		return page.getList();
	}
