...
import org.activiti.image.ProcessDiagramGeneratorMine;
 ...
 */
public class DefaultProcessDiagramGeneratorMine implements ProcessDiagramGeneratorMine {
    ...
    public DefaultProcessDiagramGeneratorMine() {
        this(1.0);
    }

    // The instructions on how to draw a certain construct is
    // created statically and stored in a map for performance.
    public DefaultProcessDiagramGeneratorMine(final double scaleFactor) {
        // start event
        activityDrawInstructions.put(StartEvent.class, new ActivityDrawInstruction() {

            public void draw(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode) {
        ...
        artifactDrawInstructions.put(TextAnnotation.class, new ArtifactDrawInstruction() {
            ...
            public void draw(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel, Artifact artifact) {
    ...
    @Override
    public InputStream generateDiagram(BpmnModel bpmnModel, String imageType, List<String> currentActivity, List<String> highLightedActivities, List<String> highLightedFlows, String activityFontName, String labelFontName, String annotationFontName, ClassLoader customClassLoader, double scaleFactor) {
        return generateProcessDiagram(bpmnModel, imageType,  currentActivity,highLightedActivities, highLightedFlows,
                activityFontName, labelFontName, annotationFontName, customClassLoader, scaleFactor).generateImage(imageType);
    }

    protected DefaultProcessDiagramCanvasMine generateProcessDiagram(BpmnModel bpmnModel, String imageType,
                                                                     List<String> highLightedActivities, List<String> highLightedFlows,
                                                                     String activityFontName, String labelFontName, String annotationFontName, ClassLoader customClassLoader, double scaleFactor) {

        prepareBpmnModel(bpmnModel);

        DefaultProcessDiagramCanvasMine processDiagramCanvas = initProcessDiagramCanvas(bpmnModel, imageType, activityFontName, labelFontName, annotationFontName, customClassLoader);
    ...
    protected DefaultProcessDiagramCanvasMine generateProcessDiagram(BpmnModel bpmnModel, String imageType, List<String> currentAcitity,
                                                                     List<String> highLightedActivities, List<String> highLightedFlows,
                                                                     String activityFontName, String labelFontName, String annotationFontName, ClassLoader customClassLoader, double scaleFactor) {
        ...
        for (Process process: bpmnModel.getProcesses()) {
            for (FlowNode flowNode : process.findFlowElementsOfType(FlowNode.class)) {
                drawActivity(processDiagramCanvas, bpmnModel, flowNode, currentAcitity,highLightedActivities, highLightedFlows, scaleFactor);
            }
        }
    ...
    protected void drawActivity(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel,
    ...
    protected void drawActivity(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel,
                                FlowNode flowNode, List<String> currentActivity, List<String> highLightedActivities, List<String> highLightedFlows, double scaleFactor) {
        ...
        if (drawInstruction != null) {
            ...
            if (currentActivity.contains(flowNode.getId())) {
                drawCurrentLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
            }
            //drawCurrentLight(processDiagramCanvas,bpmnModel.getGraphicInfo(flowNode.getId()));
        ...
        if (flowNode instanceof FlowElementsContainer) {
            for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
                if (nestedFlowElement instanceof FlowNode) {
                    drawActivity(processDiagramCanvas, bpmnModel, (FlowNode) nestedFlowElement,
                            currentActivity,highLightedActivities, highLightedFlows, scaleFactor);
                }
            }
        }
    }
     ...
     * @return
     */
    protected static List<GraphicInfo> connectionPerfectionizer(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel, BaseElement sourceElement, BaseElement targetElement, List<GraphicInfo> graphicInfoList) {
        ...
        DefaultProcessDiagramCanvasMine.SHAPE_TYPE sourceShapeType = getShapeType(sourceElement);
        DefaultProcessDiagramCanvasMine.SHAPE_TYPE targetShapeType = getShapeType(targetElement);
     ...
     */
    protected static DefaultProcessDiagramCanvasMine.SHAPE_TYPE getShapeType(BaseElement baseElement) {
        if (baseElement instanceof Task || baseElement instanceof Activity || baseElement instanceof TextAnnotation) {
            return DefaultProcessDiagramCanvasMine.SHAPE_TYPE.Rectangle;
        } else if (baseElement instanceof Gateway) {
            return DefaultProcessDiagramCanvasMine.SHAPE_TYPE.Rhombus;
        } else if (baseElement instanceof Event) {
            return DefaultProcessDiagramCanvasMine.SHAPE_TYPE.Ellipse;
        } else {
    ...
    protected void drawArtifact(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel, Artifact artifact) {
    ...
    private static void drawHighLight(DefaultProcessDiagramCanvasMine processDiagramCanvas, GraphicInfo graphicInfo) {
        processDiagramCanvas.drawHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());

    }

    private static void drawCurrentLight(DefaultProcessDiagramCanvasMine processDiagramCanvas, GraphicInfo graphicInfo) {
        processDiagramCanvas.drawCurrentLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());

    }

    protected static DefaultProcessDiagramCanvasMine initProcessDiagramCanvas(BpmnModel bpmnModel, String imageType,
        ...
        return new DefaultProcessDiagramCanvasMine((int) maxX + 10,(int) maxY + 10, (int) minX, (int) minY,
    ...
    protected interface ActivityDrawInstruction {
        void draw(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode);
    }

    protected interface ArtifactDrawInstruction {
        void draw(DefaultProcessDiagramCanvasMine processDiagramCanvas, BpmnModel bpmnModel, Artifact artifact);
    }
}
