 ...
 * @version 2017-10-30
 */
public class TestTaskRecord extends DataEntity<TestTaskRecord> {
	...
	private String taskExeId;		// 任务执行ID
	private String sampleCode;		// 样品编号
	private String templateId;		// 原始记录单模版
	private String fileId;		// 原始记录单
	private Integer status;		// 原始记录单状态
	
	public TestTaskRecord() {
		super();
	}

	public TestTaskRecord(String id){
		super(id);
	}
	...
	@Length(min=0, max=64, message="任务执行ID长度必须介于 0 和 64 之间")
	...
	@Length(min=0, max=64, message="原始记录单模版长度必须介于 0 和 64 之间")
	...
	@Length(min=0, max=64, message="原始记录单长度必须介于 0 和 64 之间")
